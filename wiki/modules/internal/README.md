...
The Code-Reducer static analysis pipeline is composed of four internal packages that orchestrate an end-to-end automated documentation generation process for software repositories. The data flow follows this sequence: **repository discovery** (`tools`) → **path safety validation** (`security`) → **configuration resolution** (`config`) → **orchestration and synthesis** (`engine`). All external dependencies funnel through the `llmCaller` interface defined in `internal/config`, enabling transport-level abstraction without coupling to a specific LLM provider.

| Package | Responsibility | Key Interface Boundary |
|---|---|---|
| `internal/config` | Runtime configuration schema, YAML persistence with atomic semantics, multi-source resolution (YAML → env → CLI flags → defaults) | `*Config`, `llmCaller` interface definition |
| `internal/engine` | End-to-end pipeline: file discovery, change classification, recursive LLM synthesis, markdown persistence | `Runner.Run()`, `orchestrator` package-private type |
| `internal/security` | Path traversal prevention (`SafeResolve`), cross-process exclusive locking via atomic file-based mechanism | `*SimpleLock`, sentinel errors `ErrPathTraversal`, `ErrLockHeld` |
| `internal/tools` | Safe file I/O (TOCTOU protection, atomic writes), git command abstraction for subprocess execution | `WriteFileAtomic`, `RunGit`, `DiscoverCodeFiles` |
...
```go
func WriteFileSafely(repoRoot string,