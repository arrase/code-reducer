...
Code-Reducer is an interactive LLM-driven static analysis pipeline that generates hierarchical markdown documentation for software repositories. The entry point (`main.go`) delegates to `cmd.RootCmd.Execute()`, which routes execution through a lifecycle of three engine modes—`init`, `update`, and an additional mode resolved by `executeCommand`. All processing occurs in the `internal/` packages; no domain logic is implemented at the top level.
