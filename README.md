...
Code-Reducer can be configured to document not only software codebases but also Infrastructure-as-Code (IaC) or cloud topology. You can inspect an example config tailored for Terraform project analysis in [examples/terraform/.code-reducer.yaml](file:///home/arrase/Develop/code-reducer/examples/terraform/.code-reducer.yaml).
